...
| **待回答** | `QUESTION` | 青 `#3fd8d0` | 问题标题（面板自己的 `1 / N` 题号也会留在行尾） | **挂起** |

### 瞬时 vs 挂起，本质不同

- **瞬时型**：一件事发生了 → 弹 `TOAST_MS` 就走。
- **挂起型**：**agent 正卡着等你**。它不会"5 秒后就没事了"，所以：整块显示 `PENDING_FULL_MS` →
  **收成右上角那条细边**（只表示"有东西在等你"，不带文字）→ **直到你处理掉才消失**。
  细边的颜色告诉你等的是哪一类（橙=授权，青=提问），**鼠标移上去会把板子重新展开**。

挂起型**和其余状态守同一条可见性规则**：正看着 DSH 时只出板子，没看着才追加系统通知。
（这里曾经给挂起型开过例外，理由是"agent 卡着等你更该被知道"——真机用过一次就否掉了：
正盯着那块大字板子，同时又弹一条系统通知，纯属重复。）
...
- **DSH 版本**：针对 `0.1.5-rc.x` 开发与验证。它依赖的内部接口有两类——会话快照的
  字段名，以及**官方面板的三个 data 属性**（`data-approval-key` / `data-question-key`
...
```bash
npm test                 # 三个 harness，共 191 条断言
node _mutate.js list     # 列出 24 个变异体，各自对应哪条断言
```
...
| 来源 | 说明 |
| --- | --- |
| `session.getSnapshot().lastAgentError` | **唯一**来源。它喂的是宿主那条 `api-session/error` 事件（DSH 官方注释称其为"**没有 turn 位置的实时失败**"的出口），但插件读的是**快照上的值**，没订阅那个事件 |
...
那一行的文案 = 面板的 `textContent` 依次减去三类噪音：

1. **面板自己的固定文案**（`等待审批`/`拒绝`/`允许一次`/`跳过本题`/`确认执行`…，取自官方字典）。
   `计划待审` **故意不剥**——它是有意义的状态标签，不是按钮。
2. **成对的 Markdown 标记**（`**x**`、`` `x` ``）——那一行是纯文本 HUD，不解释语法，不剥就会
   原样显示星号。**单星号 `*x*` 刻意不处理**：它和通配符、乘法撞得太死，控制测试当场抓到过
   `排除 *.png 和 2*3` 会被吃成 `排除 .png 和 23`。下划线与波浪号也不碰（标识符和路径里太常见）。
3. **行尾的短按钮标签**（`提交`/`跳过`/`取消`，`提交` 来自 DSH 公共词表而非面板自己的字典）。
   这些是**普通词**，只能切行尾——按钮在 DOM 里本来就是最后一个，所以 `提交前要检查什么`
   里的"提交"不会被误删。
...
node verify-status.js    # 多状态：六个状态、挂起生命周期、DOM 去重、展示行清理、优先级
```
...
- **没看着才发通知**：三个状态各有一条"正在看着 → 系统通知数是 0"的断言，加一条
  ...
  所以现在有变异体专门盯着它不许偷偷豁免。
- **展示行清理带反例控制**：剥 Markdown 和行尾标签各有"必须剥掉"与"绝不能误伤"两个方向的断言
  （`*.png`/`2*3`/`file_name` 必须原样保留，`提交前要检查什么` 里的"提交"必须留下）。
...
| `no-markup-strip` | 让字面星号显示在板子上 | 1 条 |
| `markup-strip-too-greedy` | 无差别删星号，毁掉通配符与乘法 | 2 条 |
| `no-tail-strip` | 把提交按钮留在行尾 | 1 条 |
| `tail-strip-anywhere` | 行尾标签改成全局删，吃掉问题正文里的同一个词 | 2 条 |
...
变异测试在这里不是形式，它抓出的每一类都是我自己写的问题：一段**不可达**的防御代码（删掉了）、
一个**打空了的** reduced-motion 规则（动画搬到 `::after` 之后没跟着搬）、**一条拿 harness
自己的常量去比的不变式**（所以 `pointer-target-overlaps-plate` 一开始活了下来）、
**一个只写一次的高度**（第一次挂起时细边先于板子高度测量被创建，于是永远停在退路值上），
以及**一条过度剥离的清理规则**（无差别删星号，把 `*.png 和 2*3` 吃成 `.png 和 23`）。
